...
class BNBPineScriptStrategy(Strategy):
    """
    Strategia dla BNBUSDT przetłumaczona z PineScript v6.
    
    Parametry z PineScript:
    - num_falling: 5 (liczba spadkowych świeczek)
    - allow_one_break: True (pozwól jedną zaburzającą świeczkę)
    - take_profit_perc: 4.0% (TP – start śledzenia)
    - stop_loss_perc: 6.0% (Stop Loss)
    - red_candles_to_sell: 6 (ilość czerwonych świeczek do sprzedaży po TP)
    - loss_lookback_bars: 6 (ilość świeczek do blokady kupna po stracie)
    
    Logika:
    - Kupno: 5 spadkowych świeczek (z opcjonalnym 1 zaburzeniem)
    - Blokada kupna po stracie w ostatnich 6 świeczkach
    - Stop Loss: sztywny 6%
    - Take Profit: śledzony - aktywuje się po osiągnięciu 4%, 
      następnie czeka na 6 czerwonych świeczek przed sprzedażą
    """
    ...
    def __init__(self, symbol: str, params: dict, strategy_id: str = None):
        ...
        self.num_falling = params.get('num_falling', 5)
        self.allow_one_break = params.get('allow_one_break', True)
        self.take_profit_perc = params.get('take_profit_perc', 4.0)
        self.stop_loss_perc = params.get('stop_loss_perc', 6.0)  # Optymalny stosunek TP:SL
        self.red_candles_to_sell = params.get('red_candles_to_sell', 6)
        self.loss_lookback_bars = params.get('loss_lookback_bars', 6)
    ...
    def _f_check_falling(self, df: pd.DataFrame, num: int, allow_break: bool) -> bool:
        ...
        f_check_falling(num, allow_break) =>
            falling_count = 0
            break_used = false
            for i = 1 to num + (allow_break ? 1 : 0)
    ...
    def check_sell_signal(self, df: pd.DataFrame, position) -> tuple[bool, str]:
        ...
        # === TAKE PROFIT (śledzony, 4% trigger) ===
        ...
        if position.tp_tracking:
            ...
            #     red_count := 0
            #
            # WAŻNE: W PineScript to działa na BIEŻĄCEJ świecy
            # Musimy sprawdzić czy OSTATNIA (najnowsza) świeca jest czerwona
            ...
            # Sprzedaż po N czerwonych świeczkach (6 w tym przypadku)
    ...
    def get_take_profit(self, entry_price: float) -> float:
        """
        Zwraca cenę aktywacji take profit (4% trigger).
